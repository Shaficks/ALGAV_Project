...
int tailleListe=0; //Taille de la liste de mots courante. à revoir.
char* base[] = {"A","quel","genial","professeur","de","dactylographie","sommes","nous","redevables","de","la","superbe","phrase", 
		"ci","dessous",",","unmodele","du","genre","que","toute","dactylo","connait","par","coeur","puisque","elle","fait",
 ...
 */
struct Briandais {/*!< Valeur contenue dans le noeud. */
...
struct Hybride {
  char val;           /*!< Valeur contenue dans le noeud. */
  struct Hybride *upperChild; /*!< Pointeur sur le noeud fils supérieur. (Représente le caractère supérieur remplaçant le noeud courant)*/
  struct Hybirde *lowerChild; /*!< Pointeur sur le noeud fils inférieur. (Représente le caractère inférieur remplaçant le noeud courant)*/ 
...
ArbreBriandais constructArbreBriandais(char** mots); //Construit un Arbre de la Briandais à partir d'un dictionnaire donné.
ArbreBriandais ajoutMotBriandais(ArbreBriandais a, char mot[]); //Ajoute un mot à l'arbre a et retourne l'Arbre résultat.
...
ArbreBriandais A_Briandais(int i, ArbreBriandais* aList, ArbreBriandais a); //Retourne l'Arbre formé avec T en i-ème position  
...
TrieHybride constructTrieHybride(char** dictionnaire); //Construit un Trie Hybride à partir d'un dictionnaire donné.
TrieHybride ajoutMotTrie(TrieHybride t, char mot[]); //Ajoute un mot au Trie t et retourne le Trie résultat.
...
TrieHybride H_Trie(int i, TrieHybride* tList, TrieHybride t); //Retourne le Trie formé avec T en i-ème sous arbre inséré dans la liste (1 <= i <= 3). 
